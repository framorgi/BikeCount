/******************************************************************************
  Arduino pneumatic hose bike counter
  Display              :  MAX7219 8-segment array // MAX7219 library, http://www.wayoda.org/arduino/ledcontrol/index.html  
  Diff Pressure sensor :  MPX5010 DP  // Datasheet, https://www.nxp.com/docs/en/data-sheet/MPX5010.pdf
*******************************************************************************/
String ProjVersion = "0.1\n";
...
void setup()
{
  ...
  */
  ...
  uint8_t mode = InitStatic(&SW_ST1);
...
void loop()
{
  ...
  if (IsSerialCommand())
  {
    ...
    int index = 0;
    Serial.print("Got string len ");
    Serial.println(iLen);
    Serial.print("Got: [");
    Serial.print(SerialCmdString);
    Serial.println("]");
    char cToken;
    char cAction;
    char *cParam;
  ...
  if (valueEncoder_E1 != lastEncoder_E1)
  {
    ...
    if (valueEncoder_E1 > lastEncoder_E1)
      IncFreq(radioStateVector);
    else
      DecFreq(radioStateVector);
  }
...
{
  if (digitalRead(AnySwitch->pin) == LOW)
  { // switch pressed
    switch (AnySwitch->state)
    {
    case 0: /* first instance of press */
      AnySwitch->millis = ul_CurrentMillis;
      AnySwitch->state = 1;
      break;
...
//   Return static switch state
// *************************************************************
uint8_t ProcessStatic(struct swstatic_ *AnySwitch)
{
...
//   Return static switch state
// *************************************************************
uint8_t InitStatic(struct swstatic_ *AnySwitch)
{
  AnySwitch->state = digitalRead(AnySwitch->pin);
  AnySwitch->prec_state = AnySwitch->state;
  //if ((AnySwitch->state == LOW))
...
void ToggleRotaryScaling(uint8_t *radioStateVectorPtr)
{
  ...
  if (radioStateVectorPtr[1] == 0)
    radioStateVectorPtr[1] = 1;
  else
    radioStateVectorPtr[1] = 0;
